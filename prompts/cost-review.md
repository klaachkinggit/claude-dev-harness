...
- **Repo signals:**
  ...
  - `.codex/bash.log` size and command count when working in Codex.
  - Long-running subagent spawns — each pays cold-start.

## Diagnose if cost feels high
1. **Skill bloat** — inspect the provider-local mirror (`.claude/skills/` in Claude, `.codex/skills/` in Codex) and `/plugin list` where available. Every skill description loads each session. Prune anything unused for ~2 weeks, then mirror the curated set.
2. **MCP server count** — inspect Claude and Codex MCP config/list output. Each server's tool list is in-context. Disable per-project servers you're not using.
