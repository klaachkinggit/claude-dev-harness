---
description: Security review — OWASP Top 10 + secrets exposure
allowed-tools: Read, Grep, Glob, Bash
argument-hint: <path or file to scan>
---

Security scan of $ARGUMENTS. Check each category and report all findings.
...
3. **Secrets** — Hardcoded credentials in code or config:
   `grep -rE "(password|secret|token|api_key|apikey)\s*[:=]\s*['\"][^'\"]{6,}" $ARGUMENTS`
...
CRITICAL or HIGH findings block ship. Fix before /preflight.
