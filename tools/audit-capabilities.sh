...
usage() {
  cat <<'EOF'
Usage:
  tools/audit-capabilities.sh [--expect-profile vercel|supabase|stripe|figma|all]
...
Checks:
  ...
  - no user-level skills/resources left in ~/.codex, ~/.claude, or ~/.agents
EOF
}
...
failures = []
warnings = []
base = {"github", "filesystem", "git", "playwright", "sequential-thinking"}
profiles = set()
